...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        business_user = User.objects.filter(username='Kevin', type='business').first()

        if not business_user:
            self.stdout.write(
                self.style.ERROR("Business demo user 'Kevin' not found.")
            )
            return

        if Offer.objects.filter(user=business_user).exists():
            self.stdout.write(
                self.style.WARNING(
                    f"Offers already exist for user '{business_user.username}'. Seed aborted."
                )
            )
            return
        ...
        self.stdout.write(
            self.style.SUCCESS(
                f"Seed completed for '{business_user.username}': "
                f"{created_offers} offers, {created_details} offer details created."
            )
        )